...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Job started in: " + currentDirectory.getAbsolutePath() + "\n");
        ...
        try {
            ...
            if (args.length == 0)
                fout = new FileOutputStream("export.alb");
            else
                if (args.length == 1)
                ...
                for (int i=0; i<files.length; i++) {
                    ...
                    try {
                        ...
                        if (files[i].getName().toLowerCase().endsWith(".txt")) {
                            ...
                            System.out.println("Compressed "+files[i].getName() + " (" + (contents.length / 1024) + "KB, " + (compressedContent.length / 1024) + ")");
                        } else {
                            //no compression
                            doutb.writeBoolean(false); //not compressed flag
                            doutb.writeInt(contents.length);
                            doutb.write(contents);
                        System.out.println("Added "+files[i].getName() + " (" + (contents.length / 1024) + "KB)");
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    } finally {
    ...
    final public static int CRC32Table[] = {
